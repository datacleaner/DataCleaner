...
public class SynonymLookupTransformerTest {
    ...
    private final SynonymCatalog sc =
            new TextFileSynonymCatalog("my synonyms", "src/test/resources/synonym-countries.txt", true, "UTF8");

    @Test
    public void testCaseInsensitiveMathingOnEveryToken() throws Exception {
        final SynonymCatalog sc =
                new TextFileSynonymCatalog("my synonyms", "src/test/resources/synonym-countries.txt", false, "UTF8");

        final MockInputColumn<String> col = new MockInputColumn<>("my col", String.class);

        final SynonymLookupTransformer transformer = new SynonymLookupTransformer(col, sc, true, configuration);
        transformer.replaceInlinedSynonyms = true;
        transformer.init();

        assertEquals("Hello DNK DNK DNK!",
                transformer.transform(new MockInputRow().put(col, "Hello denmark dnk dk!"))[0]);
        assertEquals("DNK", transformer.transform(new MockInputRow().put(col, "dk"))[0]);
        assertEquals("Hello GBR DNK GBR.",
                transformer.transform(new MockInputRow().put(col, "Hello United KINGDOM danmark uk."))[0]);
    ...
    public void testCaseInsensitiveMathingOnCompleteExpression() throws Exception {
        ...
        assertEquals("Hello denmark dnk dk!",
                transformer.transform(new MockInputRow().put(col, "Hello denmark dnk dk!"))[0]);
        ...
        assertEquals("Hello United KINGDOM danmark uk.",
                transformer.transform(new MockInputRow().put(col, "Hello United KINGDOM danmark uk."))[0]);
    ...
    public void testTransformWithEveryToken() throws Exception {
        ...
        assertEquals("I come from GBR!",
                transformer.transform(new MockInputRow().put(col, "I come from Great Britain!"))[0]);
        final Object[] result =
        ...
        @SuppressWarnings("unchecked") final List<String> synonyms = (List<String>) result[1];
        @SuppressWarnings("unchecked") final List<String> masterTerms = (List<String>) result[2];
