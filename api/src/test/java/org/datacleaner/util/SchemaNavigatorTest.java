...
public class SchemaNavigatorTest extends TestCase {
    ...
    public void testTheBasics() throws Exception {
        ...
        assertEquals("Column[name=email,columnNumber=1,type=STRING,nullable=true,nativeType=null,columnSize=null]",
                sn.convertToColumn("email").toString());
        assertEquals("Column[name=email,columnNumber=1,type=STRING,nullable=true,nativeType=null,columnSize=null]",
                sn.convertToColumn("employees.csv.email").toString());
        assertEquals("Column[name=email,columnNumber=1,type=STRING,nullable=true,nativeType=null,columnSize=null]",
                sn.convertToColumn("resources.employees.csv.email").toString());
        assertEquals("Column[name=email,columnNumber=1,type=STRING,nullable=true,nativeType=null,columnSize=null]",
                sn.convertToColumn("resources.email").toString());
        ...
        try {
            ...
            assertEquals("Schema not-existing not found. Available schema names are: [information_schema, resources]",
                    e.getMessage());
        }
        ...
        try {
            ...
            assertEquals("No table name specified, and multiple options exist. "
                    + "Available table names are: [tables, columns, relationships]", e.getMessage());
        }
    ...
    public void testColumnNamesWithDots() throws Exception {
        ...
        final MutableTable orgTable = new MutableTable("tabl2").setSchema(schema);
        ...
        final String[] columnNames = new String[] { "SCHE.tabl2.source_id", "SCHE.tabl2.BLOB.BLOBNumberMain",
                "SCHE.tabl2.BLOB.BLOBNumberBranch" };

        final Column[] columnsResult = schemaNavigator.convertToColumns(columnNames);

        for (final Column column : columnsResult) {
            assertNotNull(column);
        }

        final List<String> columnNamesResult = CollectionUtils.map(columnsResult, new HasNameMapper());
