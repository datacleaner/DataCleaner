...
public class MLTrainingAnalyzerTest {
    ...
    private static final String[] RANDOM_WORDS = ("Lorem ipsum dolor sit amet, consectetur "
            ...
            + "occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.").split(
                    " ");
    ...
    public void testScenario() {
        ...
        analyzer.featureModifierTypes = new MLFeatureModifierType[] { MLFeatureModifierType.VECTOR_3_GRAM,
        ...
        for (int i = 0; i < 1000; i++) {
            analyzer.run(new MockInputRow().put(desc, randomDesc()).put(color, RED).put(red, randomHigh()).put(green,
                    randomLow()).put(blue, randomLow()), 1);
            analyzer.run(new MockInputRow().put(desc, randomDesc()).put(color, BLUE).put(red, randomLow()).put(green,
                    randomLow()).put(blue, randomHigh()), 1);
            analyzer.run(new MockInputRow().put(desc, randomDesc()).put(color, GREEN).put(red, randomLow()).put(green,
                    randomHigh()).put(blue, randomLow()), 1);
        }
        ...
        assertEquals(RED, metadata.getClassification(result.getTrainedClassifier().classify(MLClassificationRecordImpl
                .forEvaluation(new Object[] { randomDesc(), 255, 0, 0 })).getBestClassificationIndex()));
        assertEquals(GREEN, metadata.getClassification(result.getTrainedClassifier().classify(MLClassificationRecordImpl
                .forEvaluation(new Object[] { randomDesc(), 0, 255, 0 })).getBestClassificationIndex()));
        assertEquals(BLUE, metadata.getClassification(result.getTrainedClassifier().classify(MLClassificationRecordImpl
                .forEvaluation(new Object[] { randomDesc(), 0, 0, 255 })).getBestClassificationIndex()));
