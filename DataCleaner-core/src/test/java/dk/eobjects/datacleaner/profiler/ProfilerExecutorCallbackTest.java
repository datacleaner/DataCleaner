...
public class ProfilerExecutorCallbackTest extends DataCleanerTestCase {
	...
	public void testSplitQueries() throws Exception {
		...
		DataCleanerExecutor<ProfilerJobConfiguration, IProfileResult, IProfile> executor = ProfilerExecutorCallback
				.createExecutor();
		executor.addJobConfiguration(conf);
		executor.execute(new DataContextSelection(dataContext));

		assertEquals(result, executor.getResults().get(0).getMatrices()[0]
				.toString());

		executor = ProfilerExecutorCallback.createExecutor();
		executor.addJobConfiguration(conf);
		executor.setExecutionConfiguration(new ExecutionConfiguration(800l,
				false, false, 1, 1));
		executor.execute(new DataContextSelection(dataContext));
	...
	public void testMultipleProfileDefinitions() throws Exception {
		...
		IProgressObserver po = new IProgressObserver() {
			...
			public void notifySuccess(Table processedTable,
				...
				assertTrue(_begun.get(processedTable));
			}
		};
		executor.addProgressObserver(po);
		executor.execute(new DataContextSelection(dataContext));
		Table[] profileTables = executor.getResultTables();
		assertEquals(2, profileTables.length);

		List<IProfileResult> profileResultsForTable = executor
				.getResultsForTable(customersTable);
		assertEquals(1, profileResultsForTable.size());
		profileResultsForTable = executor.getResultsForTable(officesTable);
		assertEquals(1, profileResultsForTable.size());
	...
	public void testProfileDefinitionsWithSameTable() throws Exception {
		...
		DataCleanerExecutor<ProfilerJobConfiguration, IProfileResult, IProfile> executor = ProfilerExecutorCallback
				.createExecutor();
		executor.setExecutionConfiguration(new ExecutionConfiguration(false,
				true));
	...
	public void testOutOfMemoryError() throws Exception {
		...
		// Close once for the count, twice for the real query
		dsStrategy.close();
		dsStrategy.close();
		...
		try {
			executor.execute(new DataContextSelection(new DataContext(
					dcStrategy)));
