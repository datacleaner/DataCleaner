...
public abstract class AbstractCrosstabResultSwingRenderer<R extends CrosstabResult>
    ...
    private final class RendererCallback implements CrosstabRendererCallback<TableModel> {
        ...
        public void beginTable(final Crosstab<?> crosstab, final List<CrosstabDimension> horizontalDimensions,
            ...
            int rows = 1;
            int cols = 1;
            for (final CrosstabDimension crosstabDimension : verticalDimensions) {
                rows = rows * crosstabDimension.getCategoryCount();
            }
            rows += horizontalDimensions.size();

            for (final CrosstabDimension crosstabDimension : horizontalDimensions) {
    ...
    private static String getLabelText(final Object value) {
        if (value == null) {
    ...
    @Override
    public JComponent render(final R result) {
    ...
    protected void decorate(final R result, final DCTable table, final DisplayChartCallback displayChartCallback) {
        if (result.getCrosstab().getDimensionCount() == 2 && table.getColumnCount() > 2) {
            addDefaultBarCharts(table, displayChartCallback);
        }
    }

    protected void addDefaultBarCharts(final DCTable table, final DisplayChartCallback displayChartCallback) {
        final int rowCount = table.getRowCount();
        final int columnCount = table.getColumnCount();
        for (int i = 0; i < rowCount; i++) {
            boolean entirelyNumbers = true;
            for (int j = 1; j < columnCount; j++) {
                final String value = table.getTextValueAt(i, j);
            ...
            if (entirelyNumbers) {
                final Object firstRowCell = table.getValueAt(i, 0);
                if (firstRowCell instanceof String) {
                    final String measureName = firstRowCell.toString();
    ...
    private DCTable renderTable(final Crosstab<?> crosstab) {
