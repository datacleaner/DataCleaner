...
public class OpenDatabaseDialog extends BanneredDialog {
	...
	public OpenDatabaseDialog(DataContextSelection dataContextSelection) {
		...
		JTextArea aboutDatabases = GuiHelper.createLabelTextArea()
				.toComponent();
		aboutDatabases
				.setText("The named connections are pre-configured database connections that "
						+ "have been stored for easy reuse. To add or modify the named connections "
						+ "edit the configuration file located at "
						+ GuiConfiguration.getDataCleanerHome()
								.getAbsolutePath());
	...
	protected Component getContent() {
		...
		samplesButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (settings
						.isDriverInstalled(DatabaseDriverDialog.TERADATA_DRIVER)) {
					popup.add(sampleItem("Teradata template",
							"jdbc:teradata:<address>",
