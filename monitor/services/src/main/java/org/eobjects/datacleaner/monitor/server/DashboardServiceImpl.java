...
public class DashboardServiceImpl implements DashboardService {
	...
	@Autowired
	public DashboardServiceImpl(final Repository repository,
	...
	public List<DashboardGroup> getDashboardGroups(final TenantIdentifier tenant) {
		final RepositoryFolder timelinesFolder = _tenantContextFactory
				.getContext(tenant).getTimelineFolder();
		...
		for (RepositoryFolder folder : folders) {
			...
			final RepositoryFile descriptionFile = folder
					.getFile("description.txt");
			if (descriptionFile == null) {
				logger.debug("No description file for timeline group: {}",
						group);
			} else {
				final BufferedReader reader = new BufferedReader(
						new InputStreamReader(descriptionFile.readFile()));
				...
				try {
					...
					logger.error(
							"Error while reading timeline group description file: "
									+ descriptionFile, e);
				} finally {
	...
	@Override
	public List<TimelineIdentifier> getTimelines(final TenantIdentifier tenant,
		...
		for (RepositoryFile file : files) {
			if (file.getType() == Type.TIMELINE_SPEC) {
				String timelineName = file.getName().substring(
						0,
						file.getName().length()
								- FileFilters.ANALYSIS_TIMELINE_XML
										.getExtension().length());
				TimelineIdentifier timeline = new TimelineIdentifier(
						timelineName, file.getQualifiedPath(), group);
				result.add(timeline);
			}
		}
	...
	@Override
	public TimelineData getTimelineData(TenantIdentifier tenant,
		...
		final List<RepositoryFile> resultFiles = getResultFilesForJob(tenant,
				jobIdentifier);
		final List<TimelineDataRow> rows = new ArrayList<TimelineDataRow>();

		final HorizontalAxisOption horizontalAxisOption = timeline
	...
	private boolean isInRange(Date date,
			HorizontalAxisOption horizontalAxisOption) {
	...
	private List<RepositoryFile> getResultFilesForJob(
	...
	public TimelineDefinition getTimelineDefinition(
			final TenantIdentifier tenant, final TimelineIdentifier timeline) {
		final String path = timeline.getPath();

		logger.info("Reading timeline from file: {}", path);

		final RepositoryFile timelineNode = (RepositoryFile) _repository
				.getRepositoryNode(path);
		final TimelineReader reader = new JaxbTimelineReader();

		final TimelineDefinition timelineDefinition = reader.read(timelineNode
				.readFile());
	...
	public List<JobIdentifier> getJobs(final TenantIdentifier tenant) {
		final List<String> jobNames = _tenantContextFactory.getContext(tenant)
				.getJobNames();
	...
	@Override
	public JobMetrics getJobMetrics(final TenantIdentifier tenant,
			final JobIdentifier jobIdentifier) {
		final AnalysisJob job = _tenantContextFactory.getContext(tenant)
				.getJob(jobIdentifier.getName()).getAnalysisJob();
		...
		for (AnalyzerJob analyzerJob : analyzerJobs) {
			final Set<MetricDescriptor> metricDescriptors = analyzerJob
					.getDescriptor().getResultMetrics();
			if (!metricDescriptors.isEmpty()) {
				final String label = LabelUtils.getLabel(analyzerJob);
				final InputColumn<?> identifyingInputColumn = metricValueUtils
						.getIdentifyingInputColumn(analyzerJob);
				...
				for (MetricDescriptor metricDescriptor : metricDescriptors) {
					...
					metricIdentifier.setAnalyzerDescriptorName(analyzerJob
							.getDescriptor().getDisplayName());
					metricIdentifier.setAnalyzerName(analyzerJob.getName());
					if (identifyingInputColumn != null) {
						metricIdentifier
								.setAnalyzerInputName(identifyingInputColumn
										.getName());
					}
					metricIdentifier.setMetricDescriptorName(metricDescriptor
							.getName());
					metricIdentifier
							.setParameterizedByColumnName(metricDescriptor
									.isParameterizedByInputColumn());
					metricIdentifier
							.setParameterizedByQueryString(metricDescriptor
									.isParameterizedByString());

					metricIdentifiers.add(metricIdentifier);
				}

				final List<String> columnNames = new ArrayList<String>();
				final Set<ConfiguredPropertyDescriptor> inputProperties = analyzerJob
						.getDescriptor().getConfiguredPropertiesForInput(false);
				for (ConfiguredPropertyDescriptor inputProperty : inputProperties) {
					final Object input = analyzerJob.getConfiguration()
							.getProperty(inputProperty);
	...
	@Override
	public TimelineIdentifier updateTimelineDefinition(
			final TenantIdentifier tenant,
			final TimelineIdentifier timelineIdentifier,
			final TimelineDefinition timelineDefinition) {
		final RepositoryFile file = (RepositoryFile) _repository
				.getRepositoryNode(timelineIdentifier.getPath());

		file.writeFile(new WriteTimelineAction(timelineDefinition));
		logger.info("Updated timeline definition in file: {}", file);
		return new TimelineIdentifier(timelineIdentifier.getName(),
				file.getQualifiedPath(), timelineIdentifier.getGroup());
	}

	@Override
	public TimelineIdentifier createTimelineDefinition(
			final TenantIdentifier tenant,
			final TimelineIdentifier timelineIdentifier,
			final TimelineDefinition timelineDefinition) {
		...
		final String fileName = name
				+ FileFilters.ANALYSIS_TIMELINE_XML.getExtension();

		final RepositoryFile file = folder.createFile(fileName,
				new WriteTimelineAction(timelineDefinition));
		logger.info("Created timeline definition in file: {}", file);

		return new TimelineIdentifier(timelineIdentifier.getName(),
				file.getQualifiedPath(), group);
	}

	@Override
	public Collection<String> getMetricParameterSuggestions(
			TenantIdentifier tenant, JobIdentifier job, MetricIdentifier metric) {
		final TenantContext context = _tenantContextFactory.getContext(tenant);
		MetricValueUtils metricValueUtils = new MetricValueUtils();

		final AnalyzerBeansConfiguration configuration = context
				.getConfiguration();
		final AnalyzerBeanDescriptor<?> analyzerDescriptor = configuration
				.getDescriptorProvider()
				.getAnalyzerBeanDescriptorByDisplayName(
						metric.getAnalyzerDescriptorName());
		final MetricDescriptor metricDescriptor = analyzerDescriptor
				.getResultMetric(metric.getMetricDescriptorName());
		...
		final AnalysisResult analysisResult = context.getResult(
				resultFile.getName()).getAnalysisResult();

		final AnalysisJob analysisJob = context.getJob(job.getName())
				.getAnalysisJob();
		final AnalyzerJob analyzerJob = metricValueUtils.getAnalyzerJob(metric,
				analysisJob);

		final AnalyzerResult result = metricValueUtils.getResult(
				analysisResult, analyzerJob, metric);

		final Collection<String> suggestions = metricDescriptor
				.getMetricParameterSuggestions(result);
	...
	@Override
	public Boolean removeTimeline(TenantIdentifier tenant,
	...
	public DashboardGroup addDashboardGroup(TenantIdentifier tenant, String name) {
		...
		final RepositoryFolder timelineFolder = _tenantContextFactory
				.getContext(tenant).getTimelineFolder();
	...
	@Override
	public Boolean removeDashboardGroup(TenantIdentifier tenant,
			DashboardGroup timelineGroup) {
		final RepositoryFolder timelineFolder = _tenantContextFactory
				.getContext(tenant).getTimelineFolder();
		final RepositoryFolder groupFolder = timelineFolder
				.getFolder(timelineGroup.getName());

		if (groupFolder == null) {
			throw new IllegalArgumentException("Timeline group '"
					+ timelineGroup.getName() + "' does not exist.");
		}
		...
		try {
			...
			logger.warn("Failed to delete timeline group folder: "
					+ timelineGroup.getName(), e);
			return false;
		}
	}
