...
class DensityAnalyzerResultSwingRendererRevised extends Renderer[DensityAnalyzerResultRevised, JPanel] {
  ...
  override def getPrecedence(r: DensityAnalyzerResultRevised) = RendererPrecedence.HIGH

  override def render(r: DensityAnalyzerResultRevised): JPanel = {
    ...
    annotations.foreach(entry => {
      val x = entry._1.getX().toInt
      val y = entry._1.getY().toInt
